...
public class InternalTransitionsTest {
	...
	public static final String SELF_TRANSITION_TRIGGER = "self";
	@Mock
	...
	public void setUp() throws Exception {
		...
		builder.transition().internal(States.INITIAL).on(SELF_TRANSITION_TRIGGER).perform(transitionAction);
		...
		stateMachine.fireEvent(SELF_TRANSITION_TRIGGER);
	}
	...
	public void internalTransitionDoesNotPerformEntryActions() throws Exception {
		verifyNever().on(entryAction).onTransition(States.INITIAL, States.INITIAL, SELF_TRANSITION_TRIGGER, any(Void.class), stateMachine);
	}

	@Test
	public void internalTransitionPerformsExitActions() throws Exception {
		verifyNever().on(exitAction).onTransition(States.INITIAL, States.INITIAL, SELF_TRANSITION_TRIGGER, any(Void.class), stateMachine);
	}

	private void verifyActionInvocation(Action<States, String, Void> action) {
		verifyOnce().on(action).onTransition(States.INITIAL, States.INITIAL, SELF_TRANSITION_TRIGGER, any(Void.class), stateMachine);
	}
